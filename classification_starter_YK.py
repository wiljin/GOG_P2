	...
	for datafile in os.listdir("train"):
	    ...
	    for el in tree.iter():
	        ...
	        if el.tag == "all_section" and not in_all_section:
	            ...
	            unique_feats += list(el.attrib.items())
    
	feats_hist = Counter(unique_feats).most_common(250)
	feats = []
	for row in feats_hist:
		feats.append(row[0])
	return feats

def extract_feats(ffs, feature_list, direc="train", global_feat_dict=None):
    """
    arguments:
    ...
    for datafile in os.listdir(direc):
        ...
        # accumulate features
        [rowfd.update(ff(tree,feature_list)) for ff in ffs]
        fds.append(rowfd)
...
def histogram_system_calls(tree,featlist):
    in_all_section = False
    feat_count = Counter()
    for el in tree.iter():
    	# ignore everything outside the "all_section" element
    	if el.tag == "all_section" and not in_all_section:
    		...
    		for syscall in list(el.attrib.items()):
    			for feat in featlist:
    				if syscall == feat:
    					feat_count[syscall] += 1
    return feat_count
...
def main():
    ...
    print("finding unique features...")
    feature_list = find_unique_feats(train_dir)
    print("done finding unique features")
    print()
